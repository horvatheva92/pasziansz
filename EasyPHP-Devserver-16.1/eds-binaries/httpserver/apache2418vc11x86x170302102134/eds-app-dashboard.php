...
if ($eds_httpserver_running == 0) {
	...
	$control .= '<input type="hidden" name="action[variable][server_folder]" value="' . urlencode(basename(dirname(__FILE__))) . '" />';
	$control .= 'PHP version: ';
		...
		while (($php_folder = @readdir($php_folders)) !== false){
			if (@file_exists('../eds-binaries/php/'.$php_folder.'/eds-app-settings.php')){
				...
				if ($php_settings['app_architecture'] == 'x86' OR $php_settings['app_architecture'] == '') {
					...
					$control .= '<option value="' . urlencode($php_folder) . '">' . $php_settings['app_version'] . $tag . '</option>';
				}
			}
		}
	...
	$control .= '<select name="action[variable][server_port]" class="form-control input-sm">';
		foreach ($ports_available AS $port_available){
			$control .= '<option value="' . $port_available . '">' . $port_available . '</option>';		
		}
	$control .= '</select>';

	$control .= '<input type="hidden" name="action[request][0][type]" value="include" />';
	$control .= '<input type="hidden" name="action[request][0][value]" value="' . urlencode('../eds-binaries/httpserver/' . basename(dirname(__FILE__)) . '/eds-app-actions.php') . '" />';
	$control .= '<input type="hidden" name="action[request][1][type]" value="exe" />';
	$control .= '<input type="hidden" name="action[request][1][value]" value="' . urlencode('eds-app-launch "..\eds-binaries\httpserver\\' . basename(dirname(__FILE__)) . '\bin\eds-httpserver.exe"') . '" />';
	$control .= '<button type="submit" class="btn btn-success btn-sm" onclick="delay()" style="margin:0px 0px 0px 30px;padding-left:20px;padding-right:20px;"><span class="glyphicon glyphicon-play small" aria-hidden="true"></span>&nbsp;&nbsp;start&nbsp;</button>';
	$control .= '</form>';
	
} else {
	
	// app running - > stop app
	$control = '<form method="post" action="http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . '" class="form-inline" role="form">';
	$control .= '<input type="hidden" name="action[request][0][type]" value="exe" />';
	$control .= '<input type="hidden" name="action[request][0][value]" value="' . urlencode('eds-app-stop.exe -accepteula "eds-httpserver"') . '" />';
	$control .= '<button type="submit" class="btn btn-danger btn-sm" onclick="delay()" style="padding-left:20px;padding-right:20px;"><span class="glyphicon glyphicon-stop small" aria-hidden="true"></span>&nbsp;&nbsp;stop&nbsp;</button>';
	$control .= '</form>';
...
<?php
if ((basename(dirname(__FILE__)) !== $conf_httpserver['httpserver_folder']) AND ($conf_httpserver['httpserver_folder'] != '')) {
	...
	<div class="row">
		<div class="col-sm-10 text-center">
			...
			<form method="post" action="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ?>" class="form-inline" role="form">
				...
				<input type="hidden" name="action[request][0][value]" value="<?php echo urlencode('eds-app-stop.exe -accepteula "eds-httpserver"') ?>" />
				<input type="hidden" name="action[request][1][type]" value="exe" />
				<input type="hidden" name="action[request][1][value]" value="<?php echo urlencode('eds-app-stop.exe -accepteula "php-cgi"') ?>" />
	...
	<?php
	if ($_GET['display'] == 'serverconffile') {
		?>
	...
	if ($_GET['display'] == 'servererrorlog') {
		?>
	...
	if ($_GET['display'] == 'serveraccesslog') {
		?>
