...
if ($eds_dbserver_running == 0) {
	...
	$control .= '<input type="hidden" name="action[request][0][value]" value="' . urlencode('../eds-binaries/dbserver/' . basename(dirname(__FILE__)) . '/eds-app-actions.php') . '" />';
	$control .= '<input type="hidden" name="action[request][1][type]" value="exe" />';
	$control .= '<input type="hidden" name="action[request][1][value]" value="' . urlencode('eds-app-launch "..\eds-binaries\dbserver\\' . basename(dirname(__FILE__)) . '\bin\eds-dbserver.exe"') . '" />';
	$control .= '<button type="submit" class="btn btn-success btn-sm" onclick="delay()" style="margin:0px 0px 0px 0px;padding-left:20px;padding-right:20px;"><span class="glyphicon glyphicon-play small" aria-hidden="true"></span>&nbsp;&nbsp;start&nbsp;</button>';
	$control .= '</form>';
	...
	$control .= '<input type="hidden" name="action[request][0][value]" value="' . urlencode('"..\eds-binaries\dbserver\\' . basename(dirname(__FILE__)) . '\bin\mysqladmin.exe" -u root shutdown') . '" />';
...
<?php
if ((basename(dirname(__FILE__)) !== $eds_cfg['dbserver_folder']) AND ($eds_cfg['dbserver_folder'] != '')) {
	...
	<div class="row">
		<div class="col-sm-10 text-center">
			...
			<form method="post" action="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ?>" class="form-inline" role="form">
				...
				<input type="hidden" name="action[request][0][value]" value="<?php echo urlencode('eds-app-stop.exe -accepteula "eds-dbserver"') ?>" />
