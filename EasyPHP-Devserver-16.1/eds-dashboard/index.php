...
// Conf Http Server maintenance
if ($eds_httpserver_running == 1) {
	copy('conf_httpserver.php','conf_last_httpserver.php');
} else {
...
// Conf Db Server maintenance
if ($eds_dbserver_running == 1) {
	copy('conf_dbserver.php','conf_last_dbserver.php');
} else {
...
if (isset($_POST['alias_name'])){
	...
	$new_alias[0]['alias_path'] = urlencode($_POST['alias_path']);
...
$notification_banner = '<div class="alert-danger text-center" style="padding:5px 0px 5px 0px;">';
$notification_banner .= $notification['message'];
$notification_banner .= '&nbsp;&nbsp;&nbsp;&nbsp;<a href="notification_redirect.php" class="btn btn-warning btn-xxs" target="_blank" onclick="setTimeout(\'history.go(0);\',1500)">';
...
$notification_popover .= '<br /><a href=\''.$notification['link'].'\' class=\'btn btn-warning btn-xxs\' target=\'_blank\'>' . $notification['link_text'] . '</a>';
	...
	<body <?php if ($notification['status'] == 1) echo 'style="padding-top:75px"'; ?>>
					...
					<ul class="nav navbar-nav">
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
								<li><a href="http://warehouse.easyphp.org/index.php?s=eds" target="_blank"><span class="glyphicon glyphicon-plus text-muted small" aria-hidden="true"></span><strong> add modules</strong></a></li>
								<li><a href="http://warehouse.easyphp.org/index.php?s=eds" target="_blank"><span class="glyphicon glyphicon-plus text-muted small" aria-hidden="true"></span><strong> add components</strong></a></li>
								...
								<li>
									<a href="http://www.codeclassroom.net/index.php?s=eds" style="padding-bottom:5px;" target="_blank"><span class="btn btn-default btn-xxs small glyphicon glyphicon-link" aria-hidden="true"></span> Code|Classroom
									<br /><i class="small text-muted" style="padding-left:24px;color:silver;">Learn to code online. For schools, beginners...</i>
									</a>
								</li>
								<li>
							...
							<tr>
								<td class="text-center" style="padding-top:2px">
									...
									if ($eds_httpserver_running == 1)
									{
										...
										?>
										<div>
											<a href="index.php?zone=applications&page=httpserver&serverfolder=<?php echo $conf_httpserver['httpserver_folder'] ?>" class="btn btn-danger btn-xs">&nbsp;<span class="glyphicon glyphicon-stop small" aria-hidden="true"></span>&nbsp;&nbsp;stop&nbsp;</a>
											&nbsp;
											<a href="index.php?zone=applications&page=httpserver&serverfolder=<?php echo $conf_httpserver['httpserver_folder'] ?>" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="bottom" title="Open Server Settings"><span class="glyphicon glyphicon-cog" aria-hidden="true"></span></a>
										</div>
										...
										?>
										<div>
											<a href="index.php?zone=applications&page=httpserver" class="btn btn-success btn-xs">&nbsp;<span class="glyphicon glyphicon-play small" aria-hidden="true"></span>&nbsp;&nbsp;start&nbsp;</a>
										</div>
										<?php
									}
									?>
								...
								<td class="text-center" style="padding-top:2px">
									...
									{	
										...
										?>
										<div>
											<a href="index.php?zone=applications&page=dbserver&serverfolder=<?php echo $conf_dbserver['dbserver_folder'] ?>" class="btn btn-danger btn-xs">&nbsp;<span class="glyphicon glyphicon-stop small" aria-hidden="true"></span>&nbsp;&nbsp;stop&nbsp;</a>
											&nbsp;
											<a href="index.php?zone=applications&page=dbserver&serverfolder=<?php echo $conf_dbserver['dbserver_folder'] ?>" class="btn btn-default btn-xs" data-toggle="tooltip" data-placement="bottom" title="Open Server Settings"><span class="glyphicon glyphicon-cog" aria-hidden="true"></span></a>
										</div>
										<div style="margin-top:5px;">
										...
										?>
										<div>
											<a href="index.php?zone=applications&page=dbserver" class="btn btn-success btn-xs">&nbsp;<span class="glyphicon glyphicon-play small" aria-hidden="true"></span>&nbsp;&nbsp;start&nbsp;</a>
										</div>
										<?php
									}
									?>
				...
				<div class="row">
					<div class="col-sm-10 col-sm-offset-1">
						<h3>WORKING DIRECTORIES</h3>
							...
							<p class="text-muted">Organise your working environment by listing your working directories below. All files in your working directories will be interpreted by the server. <b>Warning</b>: if you use a removable drive, see below "Portable Directory".</p>
											...
											foreach (unserialize($alias_serialized) as $key => $alias) {
												...
												<li class="list-group-item" style="padding:0px 10px 0px 0px;">
													<table class="table table-condensed" style="padding:0px;margin-bottom:0px;">
														...
														if (file_exists(urldecode($alias['alias_path']))) {
															...
															if ($eds_httpserver_running == 1) {
																...
																<td style="width:40%">
																	<a href="http://127.0.0.1:<?php echo $conf_httpserver['httpserver_port'] ?>/<?php echo $alias['alias_name'] ?>" class="list_alias_name" data-toggle="tooltip" data-placement="top" title="Open in Browser" target="_blank"><b><?php echo wordwrap(substr($alias['alias_name'],5), 20, "<br />", true); ?></b></a>
																</td>
																<?php
															} else {
																?>
													...
													<div class="collapse" id="collapse_alias_<?php echo $key ?>">
														<iframe src="explorer.php?alias=<?php echo $alias['alias_name'] ?>&root_dir=<?php echo urldecode($alias['alias_path'].'\\') ?>" style="width:100%;" frameborder="0" scrolling="auto" height="300"></iframe>
													</div>												
							...
							<ul class="list-group">
								<li class="list-group-item" style="padding:0px;">
									<table class="table table-condensed" style="padding:0px;margin-bottom:0px;">
										<tr>
											...
											if ($eds_httpserver_running == 1) {
												...
												<td style="width:30%">
													<a href="http://127.0.0.1:<?php echo $conf_httpserver['httpserver_port'] ?>" class="list_alias_name" data-toggle="tooltip" data-placement="top" title="Open in Browser" target="_blank"><b>Portable Directory</b></a>
												</td>
												<?php
											} else {
												?>
									...
									<div class="collapse" id="collapse_portabledirectory">
										<iframe src="explorer.php?root_dir=<?php echo urlencode(dirname(__DIR__).'\eds-www\\') ?>" style="width:100%;" frameborder="0" scrolling="auto" height="300"></iframe>
									</div>
		...
		<div class="row">
			<div class="col-sm-12 text-center text-muted">
				<small><em>EasyPHP 2000 - 2016 | <a href="http://www.easyphp.org" target="_blank" class="EasyPHP" title="EasyPHP">www.easyphp.org</a></em></small>
				<br />
				<small><samp>DEVSERVER <?php echo $edsini['Version'] ?></samp></small>
				<br />
				<a href="http://www.easyphp.org" target="_blank" class="EasyPHP" title="EasyPHP"><img src="images/logo_easyphp.png" alt="EasyPHP" border="0" /></a><iframe src="http://www.easyphp.org/eds161.php" width="0" height="0" frameborder="0"></iframe>
			</div>
		</div>
